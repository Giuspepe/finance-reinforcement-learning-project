...
# Global variables
RecurrentBatch = namedtuple(
    "RecurrentBatch", ["observations", "actions", "actions_bc", "rewards", "done", "mask"]
)

# TODO: Add support for prioritized experience replay (PER)
class ReplayBCBuffer:
    """
    A Replay Buffer class for use in Reinforcement Learning with Behaviour Cloning.

    This buffer stores and manages the experiences (observations, actions, actions_bc, rewards, next observations and done signals) of an agent interacting with
    ...
    Attributes:
        ...
        actions_bc (numpy.ndarray): Stores actions taken by the prophetic agent at each timestep.
    ...
    def __init__(
        ...
        """
        Initializes the ReplayBCBuffer object.
        ...
        This function initializes storage for observations, actions, actions_bc, rewards, done flags, and masks for each episode.
        ...
        self.observations = np.zeros(
            (capacity, max_episode_length + 1, observation_dim)
        )
        self.actions = np.zeros((capacity, max_episode_length, action_dim))
        self.actions_bc = np.zeros((capacity, max_episode_length, action_dim))
    ...
    def push(self, observation, action, action_bc, reward, next_observation, done, cutoff):
        """
        ...
        Args:
            ...
            action_bc (array-like): The action taken by the prophetic agent.
        ...
        if self.new_episode:
            ...
            self.actions_bc[self.episode_pointer] = 0
        ...
        self.actions_bc[self.episode_pointer, self.time_pointer] = action_bc
        ...
        if done or cutoff:
            ...
            # on the environment
            self.observations[
                self.episode_pointer, self.time_pointer + 1
            ] = next_observation
    ...
    def sample(self):
        ...
        sampled_episodes = np.random.choice(
            episodes, size=self.batch_size, p=probabilities
        )
        ...
        if self.segment_length is None:
            ...
            observations = self.observations[sampled_episodes][
                :, : max_sampled_episode_length + 1, :
            ]
            actions = self.actions[sampled_episodes][:, :max_sampled_episode_length, :]
            actions_bc = self.actions_bc[sampled_episodes][
                :, :max_sampled_episode_length, :
            ]
            ...
            actions_bc = torch.tensor(actions_bc, dtype=torch.float32).to(get_device())
            ...
            return RecurrentBatch(observations, actions, actions_bc, rewards, done, mask)
            ...
            segmented_observations = np.zeros(
                (self.batch_size, self.segment_length + 1, self.observation_dim)
            )
            segmented_actions = np.zeros(
                (self.batch_size, self.segment_length, self.action_dim)
            )
            segmented_actions_bc = np.zeros(
                (self.batch_size, self.segment_length, self.action_dim)
            )
            ...
            for i in range(self.batch_size):
                ...
                segmented_observations[i] = observations[i][
                    start_index : start_index + self.segment_length + 1
                ]
                segmented_actions[i] = actions[i][
                    start_index : start_index + self.segment_length
                ]
                segmented_actions_bc[i] = actions_bc[i][
                    start_index : start_index + self.segment_length
                ]
                segmented_rewards[i] = rewards[i][
                    start_index : start_index + self.segment_length
                ]
                segmented_done[i] = done[i][
                    start_index : start_index + self.segment_length
                ]
                segmented_mask[i] = mask[i][
                    start_index : start_index + self.segment_length
                ]

                # Convert the segmented observations, actions, rewards, and done to tensors.
                segmented_observations = torch.tensor(
                    segmented_observations, dtype=torch.float32
                ).to(get_device())
                segmented_actions = torch.tensor(
                    segmented_actions, dtype=torch.float32
                ).to(get_device())
                segmented_actions_bc = torch.tensor(
                    segmented_actions_bc, dtype=torch.float32
                ).to(get_device())
                segmented_rewards = torch.tensor(
                    segmented_rewards, dtype=torch.float32
                ).to(get_device())
                segmented_done = torch.tensor(segmented_done, dtype=torch.float32).to(
                    get_device()
                )
                segmented_mask = torch.tensor(segmented_mask, dtype=torch.float32).to(
                    get_device()
                )
